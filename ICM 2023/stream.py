...
# 生成图表
grid = np.linspace(1995 ,2025, num=500)
y_smoothed = [gaussian_smooth(x, y_, grid, 1) for y_ in y]
ax.stackplot(grid, y_smoothed, colors=c1list, baseline="weighted_wiggle")
ax.spines['left'].set_visible(False)
ax.spines['right'].set_visible(False)
ax.set_yticks([])

plt.legend(['USA','China','Japan','Germany','UK','India','France','Italy','Canada'],ncol=9,loc='upper center',columnspacing=0.8,bbox_to_anchor=(.5, 1.1))
plt.tight_layout()
