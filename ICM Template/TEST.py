...
labels = ['600118.sh', '000519.sz', '600343.sh', '600000.sh']
...
for i in range(len(labels)):
    portfolio.insert(i, labels[i], get_close(labels[i],'2014-01-01','2022-12-31')['close'])
...
plt.scatter(pvols,prets,c=prets/pvols,marker='o',cmap='PuBu')
# # efficient frontier
plt.scatter(tvols,trets,c=trets/tvols,marker='o',cmap='PuBu')#marker
plt.plot(tvols,trets,c='black',linewidth=2)
# # portfolio with highest Sharpe ratio
plt.plot(statistics(opts['x'])[1],statistics(opts['x'])[0],'b*',alpha=0.8,markersize=15.0,label='最大夏普比率')
# # minimum variance portfolio
plt.plot(statistics(optv['x'])[1],statistics(optv['x'])[0],'r*',alpha=0.8,markersize=15.0,label='最小波动率')
# plt.grid(False)
plt.xlabel('波动率')
plt.ylabel('预期收益率')
